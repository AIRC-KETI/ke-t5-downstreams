...
`_gen`이 없는 태스크들은 Encoder 모델에 헤드를 붙여 학습시킬 수 있는 태스크들입니다.
...
**klue/bert-base** 모델을 이용하여 klue topic classification을 학습하는 경우 아래와 같이 입력합니다.
이 경우 BERT를 이용한 sequence classification 모델은 transformers 모듈의 BertForSequenceClassification 클래스입니다.
`{모듈 경로}:{클래스 이름}` 형태로 **--model_name**에 입력해줍니다. 개인이 만든 모델들도 입력과 출력이 huggingface 모델과 동일하다면 똑같이 학습시킬 수 있습니다.
(gin/klue_bert_tc.gin에는 vocabulary를 klue/bert-base의 vocabulary로 설정합니다.)


```bash
python -m torch.distributed.launch --nproc_per_node=2 train_ddp.py \
    --batch_size 16 \
    --gin_file="gin/klue_bert_tc.gin" \
    --pre_trained_model "klue/bert-base" \
    --model_name transformers:BertForSequenceClassification \
    --task 'klue_tc'
```
