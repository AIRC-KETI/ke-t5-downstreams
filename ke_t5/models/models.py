...
@register_model('T5EncoderForSequenceClassificationREMeanAttention')
class T5EncoderForSequenceClassificationREMeanAttention(T5EncoderModel):
    def __init__(self, config):
        if not hasattr(config, 'problem_type'):
            config.problem_type = None
        super(T5EncoderForSequenceClassificationREMeanAttention, self).__init__(config)
        self.num_labels = config.num_labels        
        self.model_dim = config.d_model
        ...
        self.dropout = nn.Dropout(config.dropout_rate)
        self.fc_layer = nn.Sequential(nn.Linear(self.model_dim, self.model_dim))
        self.classifier = nn.Sequential(nn.Linear(self.model_dim * 2, self.num_labels)
                                       )
