...
# 2021.06.16. bshong
# def accuracy_dict_sample_weight(gathered_dict, target_key='labels', prediction_key='predictions'):
#     targets = gathered_dict[target_key]
#     predictions = gathered_dict[prediction_key]
#     return {"accuracy_sample_weight": {'score': 100*sklearn.metrics.accuracy_score(targets, predictions, sample_weight=targets!=0), 'count': len(targets)}}
...
def f1_score_dict_micro_sample_weight(gathered_dict, target_key='labels', prediction_key='predictions'):
    ...
    return {"f1_score_micro_sample_weight": {'score': 100*sklearn.metrics.f1_score(targets, predictions, average='micro', sample_weight=sample_weight, zero_division = 0), 'count': len(targets)}}

# 2021.06.17. bshong
def f1_score_dict_micro_sample_weight_2(gathered_dict, target_key='labels', prediction_key='predictions'):
    targets = gathered_dict[target_key]
    predictions = gathered_dict[prediction_key]
    sample_weight = np.array(targets != 0, dtype=np.float)
    return {"f1_score_micro_sample_weight_2": {'score': 100*sklearn.metrics.f1_score(targets, predictions, average='micro', sample_weight=sample_weight, zero_division = 0), 'count': np.count_nonzero(targets)}}
